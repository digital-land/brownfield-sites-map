...
const datasets = [
  ...
  },
  {
    title: 'Organisations',
    type: 'organisation',
    file: path.resolve(process.cwd(), 'organisation-dataset/collection/organisation.csv')
  }
]
...
organisationStream.pipe(csv.parse({
  ...
  on_record (record) {
    ...
    const brownfield = brownfieldMapped.filter(row => row['organisation'].toLowerCase() === record['organisation'].toLowerCase())
    const resource = brownfieldMapped.find(row => row['organisation'].toLowerCase() === record['organisation'].toLowerCase())
